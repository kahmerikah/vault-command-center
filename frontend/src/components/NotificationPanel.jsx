        ...
        {items.map((item) => (
          <div key={item.id} className="rounded-lg border border-slate-700/70 bg-vault-bg/50 px-3 py-2">
            ...
            <div className="mt-2 flex gap-2 text-xs">
              {!item.is_read && (
                <button type="button" onClick={() => onRead?.(item.id)} className="rounded border border-vault-accent/30 px-2 py-1">
                  Mark read
                </button>
              )}
              {item.status !== "archived" && (
                <button type="button" onClick={() => onArchive?.(item.id)} className="rounded border border-vault-accent/30 px-2 py-1">
                  Archive
                </button>
              )}
            </div>
